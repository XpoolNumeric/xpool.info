    ...
    >
      ...
      >
        ...
        <div className="flex-1 flex justify-center items-center w-full">
          <div
            className={`relative w-full max-w-sm aspect-square ${!prefersReducedMotion ? "animate-float" : ""
              }`}
            style={{ animation: "float 6s ease-in-out infinite" }}
          >
            {/* Background dashed ring */}
            <div
              className="absolute -inset-4 rounded-[2.5rem] border-2 border-dashed border-amber-300/40"
              style={
            ...
            />
            {/* Image container */}
            <div className="absolute inset-0 rounded-[2rem] border-4 border-white/60 bg-white/40 backdrop-blur-sm shadow-2xl overflow-hidden group">
              <div className="w-full h-full relative">
                {/* Fallback gradient */}
                <div className="absolute inset-0 bg-gradient-to-br from-amber-100 to-amber-300 opacity-50" />

                {/* The actual image */}
                <img
                  src={step.image}
                  alt={step.title}
                  className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 ease-[0.22,1,0.36,1] group-hover:scale-105"
                  loading="lazy"
                />

                {/* Optional dark gradient to make badge pop */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/30 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              </div>
            </div>

            {/* Core icon (subtle floating) */}
            <div className="absolute -bottom-4 -left-4 w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center shadow-lg border-2 border-white/80 z-10">
              <step.icon size={20} className="text-white" />
            </div>

            {/* Floating step badge */}
            <div className="absolute -top-4 -right-4 w-12 h-12 rounded-full bg-gray-900 border-4 border-white text-white flex items-center justify-center text-lg font-bold font-syne shadow-lg z-10">
              {step.number}
            </div>
          </div>
        </div>
